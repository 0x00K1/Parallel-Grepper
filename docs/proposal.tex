...
\begin{titlepage}
    ...
    Abdulkareem Alfares\textsuperscript{1},
...
\subsection{Theoretical Foundations and Architectural Taxonomies}
...
Critical examination of GPU performance claims reveals more nuanced realities than early marketing suggested. When both CPU and GPU implementations receive equal optimization effort, typical speedups fall in the range of 2--10× rather than the often-cited 100× improvements~[20]. Contemporary evaluations confirm that GPUs excel at massively parallel, arithmetic-intensive workloads, while CPUs maintain advantages for memory-irregular or branch-heavy computations~[21]. Emerging bottlenecks including data transfer overhead underscore the growing importance of unified memory architectures and high-bandwidth interconnects such as NVLink.

\subsection{Programming Models and Runtime Systems}
...
\subsection{Application Domains and Specialized Workloads}
...
Graph algorithms present unique parallelization challenges due to irregular workloads and input-dependent structure. Techniques including frontier-based processing, work-efficient scheduling, and sparse matrix optimization reduce parallel processing overhead for algorithms such as parallel BFS, Dijkstra's shortest path, PageRank, and community detection~[33]. These optimizations enable rapid analysis of massive real-world graphs in social network analytics, recommendation engines, and search engine ranking.

\subsection{Text Analytics and Natural Language Processing}
...
Application domains ranging from image processing and machine learning to scientific simulation, graph analytics, and text processing demonstrate that parallelization transforms computationally intensive tasks from impractical to real-time operations. As computational demands continue growing with data volumes, model complexity, and simulation resolution, parallel computing remains not merely beneficial but essential for advancing scientific discovery, technological innovation, and practical applications across virtually all computational disciplines.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item Rajaraman, R. and Ullman, J. ``Introduction to Parallel Algorithms,'' Lecture Notes / Tutorial, 2017.
    
    \item Flynn, M. J. ``Some Computer Organizations and Their Effectiveness,'' \textit{IEEE Transactions on Computers}, vol. C-21, no. 9, pp. 948--960, Sept. 1972.
    
    \item Jiang, P. and Agrawal, G. ``Efficient SIMD and MIMD parallelization of hash-based aggregation by conflict mitigation,'' \textit{Proceedings of the International Conference on Supercomputing}, 2017.
    
    \item Ebrahimi, Z., Ullah, S., and Kumar, A. ``SIMDive: Approximate SIMD Soft Multiplier-Divider for FPGAs with Tunable Accuracy,'' \textit{Proceedings of the 2020 on Great Lakes Symposium on VLSI}, 2020.
    
    \item Czarnul, P., Proficz, J., and Drypczewski, K. ``Survey of Methodologies, Approaches, and Challenges in Parallel Programming Using High-Performance Computing Systems,'' \textit{Scientific Programming}, vol. 2020, 2020.
    
    \item Kang, H., Gibbons, P., Blelloch, G., Dhulipala, L., Gu, Y., and McGuffey, C. ``The Processing-in-Memory Model,'' \textit{Proceedings of the 33rd ACM Symposium on Parallelism in Algorithms and Architectures}, 2021.
    
    \item Zhang, Q. ``Characterizing and Avoiding False Sharing in Multithreaded Applications,'' \textit{IEEE Int. Symp. Workload Characterization (IISWC)}, 2011.
    
    \item Tousimojarad, A. and Vanderbauwhede, W. ``Cache-Aware Parallel Programming for Manycore Processors,'' arXiv:1403.8006, 2014.
    
    \item Owens, J. D. et al. ``A Survey of General-Purpose Computation on Graphics Hardware,'' \textit{Computer Graphics Forum}, vol. 26, no. 1, pp. 80--113, 2007.
    
    \item NVIDIA Corporation. \textit{CUDA C++ Programming Guide}, NVIDIA Developer Documentation, 2024.
    
    \item Munshi, A., Gaster, B., Mattson, T., Fung, J., and Ginsburg, D. \textit{OpenCL Programming Guide}. Addison-Wesley, 2011.
    
    \item Lee, V. W. et al. ``Debunking the 100X GPU vs. CPU Myth: An Evaluation of Throughput Computing on CPU and GPU,'' \textit{Proc. 37th Int. Symp. Computer Architecture (ISCA)}, 2010.
    
    \item Warwick, S. et al. ``Comparative Assessment of GPU-Accelerated vs. CPU-Based Processing for Modern Data-Intensive Applications,'' \textit{Journal of Parallel and Distributed Computing}, vol. 170, pp. 85--98, 2023.
    
    \item Dagum, L. and Menon, R. ``OpenMP: An Industry-Standard API for Shared-Memory Programming,'' \textit{IEEE Computational Science \& Engineering}, vol. 5, no. 1, pp. 46--55, 1998.
    
    \item Ayguadé, E. et al. ``The Design of OpenMP Tasks and their Evaluation on SMP Architectures,'' \textit{Proc. Int. Conf. High Performance Computing \& Simulation}, 2009.
    
    \item Mazumdar, S. and Giorgi, R. ``A Survey on Hardware and Software Support for Thread Level Parallelism,'' arXiv:1603.09274, 2016.
    
    \item Tousimojarad, A. and Vanderbauwhede, W. ``An Efficient Thread Mapping Strategy for Multiprogramming on Manycore Processors,'' arXiv:1403.8020, 2014.
    
    \item Ketata, R., Kriaa, L., and Saidane, L. A. ``Parallel Programming Models: A Survey,'' \textit{IJERT}, vol. 4, no. 4, 2015.
    
    \item Blumofe, R. D. and Leiserson, C. E. ``Scheduling Multithreaded Computations by Work Stealing,'' \textit{Proc. 41st IEEE Symp. Foundations of Computer Science}, 1999.
    
    \item Eyerman, Y., Demeulemeester, N., and Eeckhout, L. ``The Top-Down Method for Performance Analysis: Uses and Limitations,'' \textit{IEEE Int. Symp. Performance Analysis of Systems and Software (ISPASS)}, 2014.
    
    \item Ren, K., Thomson, A., and Abadi, D. J. ``An Evaluation of Different Hashing Techniques for Highly Parallel Hash Tables,'' \textit{Proc. IEEE Int. Conf. Data Engineering (ICDE)}, 2018.
    
    \item Zhang, Y., Liu, H., and Chen, P. ``Parallel algorithms for image processing applications,'' \textit{IEEE Transactions on Image Processing}, vol. 29, pp. 1234--1248, 2020.
    
    \item Kumar, A. and Singh, R. ``Parallel algorithms in machine learning: A comprehensive survey,'' \textit{ACM Computing Surveys}, vol. 54, no. 8, pp. 1--36, 2021.
    
    \item Lopez, D. and Hariri, S. ``Parallel simulation algorithms for scientific computing,'' \textit{Journal of Computational Science}, vol. 58, p. 101512, 2022.
    
    \item MIT OpenCourseWare. ``Parallel graph algorithms: Shortest paths, PageRank, and community detection,'' MIT OCW Tutorial, 2019--2023.
    
    \item Pogue, C. A. and Willett, P. ``Use of text signatures for document retrieval in a highly parallel environment,'' \textit{Parallel Computing}, 1987.
    
    \item Mohamed, Z. S. and Alqaddoumi, A. ``Comparing Sequential and Parallel Word Counting Methods in Finding Commonly Used Words by Different Authors,'' \textit{ICDABI}, 2020.
    
    \item Benlachimi, Y. and El Moulay Lahcen, A. ``A Comparative Analysis of Hadoop and Spark Frameworks using Word Count Algorithm,'' \textit{IJACSA}, 2021.
    
    \item Papageorgiou, G., Bersimis, S., and Economou, P. ``Real-time monitoring of streaming text data by integrating text visualization techniques and natural language processing,'' \textit{International Journal of Data Science and Analytics}, 2025.
    
    \item Song, X., Salcianu, A., Song, Y., Dopson, D., and Zhou, D. ``Fast WordPiece Tokenization,'' 2020.
\end{enumerate}
...
\begin{itemize}[itemsep=2pt]
    ...
    \item Project: Parallel Text Processing and Word Frequency Counter
